...
import { librariesApi } from "../api/librariesSlice";
// ----------------------------------------------------------

const initialState = {
    user: helpers.isJsonString(window.localStorage.user)
    ...
    async ({ code, password, confirmPassword }) => {
        try {
            const response = await axiosPublic.post(
                ...
                { token: code, password, confirmPassword }
            );
            return response.data;
        } catch (err) {
            console.log(err.message);
        ...
        builder
            ...
            .addCase(login.fulfilled, (state, action) => {
                ...
                if (action.payload) {
                    ...
                    librariesApi.util.invalidateTags(["Libraries"]);
                }
            })
